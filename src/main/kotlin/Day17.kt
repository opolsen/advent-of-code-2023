class Day17 {
    fun solve1(lines: List<String>): Int {
        return getMinDistance(lines, 0, 3)
    }

    fun solve2(lines: List<String>): Int {
        return getMinDistance(lines, 4, 10)
    }

    private fun getMinDistance(lines: List<String>, min: Int, max: Int): Int {
        ...
        while (todos.isNotEmpty()) {
            ...
            if (current.pos == goal) {
                ...
                if (current.path.reversed().take(min).count { it == currentDir } == min) {
                    return current.distance
                }
            }
            getValidNeighbours(current, min, max).filter { it.first in positions }.forEach { neighbour ->
